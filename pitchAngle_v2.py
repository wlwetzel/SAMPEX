...
def eqPitch(eq_B, local_B,angle):
    pitch =  np.arcsin( np.sqrt(np.sin(angle*3.14159/180)**2 * eq_B / local_B))
    return pitch
...
def findPitches(times,interpolate=True):
    ...
    'A32', 'A13', 'A23', 'A33','B_X','B_Y','B_Z','B_Mag','Equator_B_Mag','Loss_Cone_2'])
    eq_B = df['Equator_B_Mag'].to_numpy()
    local_B = df['B_Mag'].to_numpy()
    ...
    det_1_Alpha = eqPitch(eq_B,local_B,-8.879 - mag_angle)
    det_1_Beta = eqPitch(eq_B, local_B, 32  - mag_angle)
    det_2_Alpha = eqPitch(eq_B, local_B, -17.351 - mag_angle)
    det_2_Beta = eqPitch(eq_B, local_B, 25.110 - mag_angle)
    det_3_Alpha = eqPitch(eq_B, local_B, -25.110 - mag_angle)
    det_3_Beta = eqPitch(eq_B, local_B, 17.351 - mag_angle)
    det_4_Alpha = eqPitch(eq_B, local_B, -32 - mag_angle)
    det_4_Beta = eqPitch(eq_B, local_B, 8.879 - mag_angle)

    d = {'det_1_Alpha' : det_1_Alpha , 'det_1_Beta' : det_1_Beta,
         'det_2_Alpha' : det_2_Alpha , 'det_2_Beta' : det_2_Beta,
         'det_3_Alpha' : det_3_Alpha , 'det_3_Beta' : det_3_Beta,
         'det_4_Alpha' : det_4_Alpha , 'det_4_Beta' : det_4_Beta,
         'LossCone' : df['Loss_Cone_2'].to_numpy()}
...
def peakFind(start,end):
    ...
    if data.empty:
        ...
        return emptyDF,emptyDF,emptyDF,emptyDF
    ...
    peakTimes = peakDF.index.values
    pitchAtPeaks = pitchInfo.loc[peakTimes]

    return data,burstDF,peakDF,pitchAtPeaks
...
if __name__ == '__main__':
    ...
    #%%
    i=1
    with open('/home/wyatt/Documents/SAMPEX/histData.csv','a') as file:
